% calculate uncertainty of modelled lifetime to emissions
clear all
...
lifetime = NaN([46, 72, yr_len*12], 'double');
...
for i = 1:length(ratios)
    ...
    for y = yr
        ...
        for m = 1:12
            ...
            un(:,:,(y-yr_sta)*12+m) = e .* grid_area .* (map_land > 0) * 3600 * 24 * 365/12; % kg month-1 
            
        end
        
        E_mon(E_mon == 0) = nan;
        E_yr(:,:, y-yr_sta+1) = nanmean(E_mon, 3) .* (map_land > 0) .* grid_area * 3600 * 24 * 365/1E12 * 1E+3; % Tg yr-1
        
        
    end
    
    save([path, 'uncertainty_lifetime_', num2str(ratio*100), '%_', num2str(yr_sta) '-', num2str(yr_end), '.mat'], 'un')
    E_yr(E_yr == 0) = NaN;
