% seasonal mean difference spatial distribution of GEOS-Chem and IASI adjusted emissions
clear all
...
d = nanmean(nanmean(iasi(:,:,mons,:) - geo(:,:,mons,:), 4),3).* (map_land > 0);
d(d == 0) = nan;
...
for sea = 1:4
    ...
    space(1 : 150) = 32; 
%     title(strcat('Adjusted - GEOS-Chem emissions (10^{-11} kg/m^{2}/s)', space(1:sp), season_name(sea,:)), 'FontSize' , 15 , 'FontName' ,'Times New Roman'); 
