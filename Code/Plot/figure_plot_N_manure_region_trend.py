'''draw agriculture N manure (trend) over different region''' 
import numpy as np
...
## N 
data = nc.Dataset(path + 'N_manure_' + str(yr_sta) + '-'+ str(yr_end) + '.nc')['N_manure'][:] # kg
data = data * land * mul
data[data == 0] = np.NaN

data_coun = pd.read_csv(path + 'FAOSTAT_data_5-8-2021.csv')
...
for i in range(len(res_name)):
    ...
    if re == 'USA':
        ni = data_coun[(data_coun['Area Code'] == 'USA')].groupby('Year').agg({'Value':'sum'}).Value*mul
    elif re == 'CHN':
        ni = data_coun[(data_coun['Area Code'] == 'CHN')].groupby('Year').agg({'Value':'sum'}).Value*mul
    else:
...
ax.set_title('Manure')   
fig.text(0.06, 0.3, 'Amount excreted (Mt annual)', rotation = 90)
plt.savefig(fname = 'C:\\Users\\Administrator\\Desktop\\output\\plot_annual_N_manure.svg', format = 'svg', bbox_inches = 'tight')
