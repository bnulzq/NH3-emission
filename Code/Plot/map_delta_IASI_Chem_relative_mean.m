% mean spatial distribution of relative concentration difference between IASI and GEOS-Chem
clear all
...
for y = yr
    ...
    for m = 1:12
        ...
        D(:, :, (y-yr_sta)*12 + m) = (iasi-geo)./iasi;

    end
end

D = abs(nanmean(D, 3)) .* map_land;
...
space(1 : 150) = 32; 
title(strcat('IAIS - GEOS-Chem (%)', space(1:sp), num2str(yr_sta), '-', num2str(yr_end)), 'FontSize' , 15 , 'FontName' ,'Times New Roman'); 
axis normal;
