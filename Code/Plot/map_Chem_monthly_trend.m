% monthly trend spatial distribution of GEOS-Chem simulation
clear all
...
for yr = years
    ...
    for mon = 1:12
        ...
        data(:, :, mon, yr - yr_sta +1) = ncread([path, 'GEOS-Chem_', year, month, '.nc'], ['GEOS-Chem daytime simulation_', year, month])';
...
for i = 1:13
    ...
    if i == 13
        ...
        title(strcat('GEOS-Chem (10^{14} molecules cm^{-2} yr^{-1})', space(1:25), 'All'), 'FontSize' , 15 , 'FontName' ,'Times New Roman'); 
        % rectangle
...
%         annotation(fig, 'rectangle', [0.30 0.24 0.12 0.325], 'Color', [1 0 1], 'LineWidth', 2); % Southern America
        ...
        title(strcat('GEOS-Chem (10^{14} molecules cm^{-2} yr^{-1})', space(1:30), mons(i)), 'FontSize' , 15 , 'FontName' ,'Times New Roman'); 
        % rectangle
